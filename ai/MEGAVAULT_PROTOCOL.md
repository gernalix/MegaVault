...
Quando ChatGPT deve creare da remoto un nuovo prompt canonico in `gernalix/codex-roadmap` e non può eseguire direttamente la CLI locale, deve usare il bridge già dedicato del registry; è vietato inventare l'ID o sostituire l'allocazione con una Issue informativa.

Flusso obbligatorio:

1. scrivere una richiesta `command=allocate` in `.github/prompt-id-request.json`, con `request_id` univoco, `source` e `project_id`/parent quando pertinenti;
2. attendere che `.github/prompt-id-response.json` riporti lo stesso `request_id`, `status=allocated` e il PROMPT_ID assegnato dal registry;
3. usare quell'ID nella mutation `register` del single writer di `codex-roadmap`; non dichiarare ancora conclusa la creazione;
4. dopo che il writer roadmap ha creato il file prompt canonico su `main`, inviare una seconda richiesta `command=materialize` con lo stesso PROMPT_ID e `content_url` del file canonico;
5. attendere la risposta corrispondente `status=materialized`; solo allora la materializzazione remota è completa.

Una response con `request_id` diverso è stale e non può essere riusata. Se il bridge non produce la conferma richiesta, fermarsi con stato pending/blocked: non generare un ID manualmente, non derivarlo dall'Issue number e non creare un registry parallelo.
