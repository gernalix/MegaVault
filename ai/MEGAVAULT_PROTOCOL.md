...
- Ogni backfill storico deve usare `source=historical-*`. Le righe con tale prefisso sono riserve terminali: restano `allocated` solo per compatibilita' dello schema, ma `materialize` e `cancel` devono rifiutarle. La fonte e' immutabile, quindi un ID storico non puo' essere trasformato in un prompt nuovo.
