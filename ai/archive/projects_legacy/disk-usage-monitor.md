...
branch=master
updated=2026-06-13 prompt_938274
...
prompt_731584=Disk Usage Monitor correctly exposed T7/root +157GiB; root cause external to monitor was /home/daniele/.codex/logs_2.sqlite-wal at 152.44GiB with Codex SQLite WAL locks; local codex-sqlite-wal-maintenance timer installed.
prompt_938274=postmortem confirmed Codex logs_2.sqlite journal_mode=wal, wal_autocheckpoint=1000, quick_check=ok, active Codex DB/WAL/SHM holder, no single runaway error loop; maintenance script now uses soft=1GiB hard=5GiB critical=20GiB with dry-run, quick_check, holder logging, SQLite checkpoint only, and optional Telegram via existing helper/env.
...
dnb=do not attribute large root deltas to backups before checking same-filesystem top files; Codex WAL incidents need lsof+sqlite checkpoint proof and must not use rm on open WAL/SHM files or generic pkill/killall codex.
...
ROAD:
now=v10 docs; canonical 3-disk whitelist, clean Telegram, compact Kuma heartbeat, status/dry-run, resilient timer+oneshot, 1GiB delta notification baseline, and Codex WAL incident linkage verified for prompt_938274.
