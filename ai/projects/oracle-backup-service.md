...
backup=scripts/backup.sh:4:ENV_FILE="/etc/oracle_backup/oracle_backup.env"; scripts/backup.sh:5:STATE_DIR="/var/lib/oracle_backup"
incident_db=/home/ubuntu/sync_root/db/incident_registry.sqlite;tool=/opt/oracle_backup/incident_registry.py;tables=incidents,incident_events
quota=LOCAL_FALLBACK_MAX_GB=7;LOCAL_FALLBACK_SOFT_PCT=80;LOCAL_FALLBACK_PREWRITE_RESERVE_GB=1;LOCAL_FALLBACK_ROOT_MIN_FREE_GB=1;state=/var/lib/oracle_backup/local_fallback_quota_state.json
...
ops=2026-06-13 prompt 486219 fix: `/opt/oracle_backup/backup.sh` enforces local fallback hard quota before restic write; initial default was `LOCAL_FALLBACK_MAX_GB=5`, superseded by prompt 847261 to current 7G; blocked state writes `BACKUP_BLOCKED_FALLBACK_QUOTA` and `/var/lib/oracle_backup/local_fallback_quota_*`.
...
ops=2026-06-13 prompt 847261 supersedes local fallback quota value from 5G to 7G after measured complete stream run of 4.71GiB plus 1G prewrite reserve; old 5G value would block the next valid fallback run.
ops=2026-06-13 prompt 847261 root cause: OCI bucket `bucket-20260206-0730` still uses 22.262GBytes/7474 objects over assumed 22GiB limit; `oraclevm/` uses 22.223GBytes and `oraclevm/data` 22.222GBytes; remote repo has empty `index/`, 837 snapshots, and `restic check --no-lock` reports unindexed packs/missing trees, so standard retention cannot safely repair it without quota headroom or destructive repo reset.
ops=2026-06-13 prompt 847261 fix: backup now checks remote/fallback destination before SQLite snapshots, forces stream-to-restic when remote unavailable, lowers local stream retention to one complete 11-snapshot run, removes retained on-disk SQLite snapshots, and sets observed fallback hard quota to 7G because a complete stream run is ~4.71GiB plus 1G reserve.
ops=2026-06-13 prompt 847261 validation: manual backup rc=0 `REMOTE_DEGRADED` via `/var/lib/oracle_backup/emergency_repo`; `/` 66% with 16G free; sqlite_snapshots 40K; emergency_repo 4.8G; local_fallback_quota_status=OK; healthcheck/monitor dry-runs rc=0 WARNING/REMOTE_DEGRADED; all oracle-backup timers active and no failed units.
ops=2026-06-13 prompt 847261 incident_registry: SQLite `/home/ubuntu/sync_root/db/incident_registry.sqlite` created with incidents `OCI_STORAGE_LIMIT_EXCEEDED` OPEN first_seen=2026-06-05T06:07Z occurrence_count=20, `OCI_REMOTE_REPOSITORY_CORRUPT` OPEN, `ORACLE_ROOT_DISK_PRESSURE` RESOLVED.
ROAD:
now=UNKNOWN
next=UNKNOWN
later=UNKNOWN
...
open=2026-06-13 prompt 847261: OCI remote remains unresolved and not technically eliminable without quota headroom/plan change or destructive reset of the corrupt `oraclevm` prefix; local backup path is functional and bounded.
