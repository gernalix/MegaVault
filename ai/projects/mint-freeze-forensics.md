...
core=snapshot(),record_sample(),daemon(),guardian(),human_report(),io_report(),recent(),weekly_report(),dashboard_summary(),score_event_causes(),adaptive_capture
...
proc_io_delta=/proc/<pid>/io read/write/cancelled-write deltas + /proc/<pid>/stat major fault deltas; normal rate-limited; forced only during adaptive capture
...
FLOW:
sample=snapshot->capture_trigger_check->boot_check->gap_check->critical_event_check->history_point->append_jsonl->prune/rotate->Kuma push
...
version=1.3.0 in CLI
security=Kuma URLs in ~/.config/mint-freeze-forensics/kuma.env only
perf=Nice=10; IOSchedulingClass=idle; v1.3 normal sample measured 1.39s/27.5MB temp baseline vs v1.2 1.64s/27.1MB; forced capture sample 1.50s/28.1MB
BUILD:
...
ROAD:
now=observe live freeze patterns for 24-72h with v1.3 adaptive capture
next=run io-report after next real freeze; confirm capture window contains read/write deltas, major faults, T7 state, kernel storage/USB evidence
