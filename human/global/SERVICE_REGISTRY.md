...
`espanso.service` e' il nuovo autostart per l'espansione testo: unita' utente abilitata su `default.target`, attiva con `ExecStart=/usr/bin/espanso daemon`, restart `on-failure` e nessuna porta aperta. La configurazione e' in `~/.config/espanso`.
