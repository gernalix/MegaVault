...
- ActivityWatch v0.13.2 e' installato dallo ZIP Linux x86_64 ufficiale GitHub in `~/.local/opt/activitywatch`. La configurazione finale e' un solo servizio user `activitywatch.service` che avvia `aw-qt --no-gui --autostart-modules aw-server`: API e dashboard locali funzionano su `127.0.0.1:5600`, crash recovery e DB sono verificati. Su GNOME Wayland i watcher standard AFK/window non sono attivi per limiti X11 verificati; stato finale `PENDENTE_WAYLAND_WATCHERS` (attivita' `284617`).
