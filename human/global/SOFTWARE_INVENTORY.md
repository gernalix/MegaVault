...
- Fedora System Monitor 1.3.1 e' installato in `/usr/local/libexec/fedora-system-monitor` con CLI `/usr/local/bin/fedora-system-monitor`; include pressione memoria composta, SMART dettagliato, salute batteria/Btrfs, dashboard e notifiche filesystem. Le capacità 1.2.0 dell'attività 962417 restano presenti senza downgrade.
- Fedora Diagnostics 1.2.0 e' installato in `/usr/local/lib/fedora-diagnostics` con CLI `/usr/local/bin/fedora-diagnostics`; `--fan-analysis` esporta storico CPU/GPU/ventole/potenza/processi e il collector textfile non legge command line o contenuti personali.
...
- Smartmontools 7.5, nvme-cli 2.16, lm_sensors 3.6 e SQLite 3.51 erano gia' disponibili. Snapper 0.13.0, btrfsmaintenance 0.5.2 e il plugin actions DNF5 5.4.2.1 gestiscono snapshot root, retention, scrub e snapshot pre-transazione. L'attivita' 593184 ha aggiunto i pacchetti Fedora Python Socket.IO e compressione necessari soltanto al provisioning amministrativo Kuma.
...
- AutoKey for Wayland e' stato rimosso nell'attivita' `582941` dopo il loop `uinput/evdev ENODEV`; pacchetti, servizio utente, launcher, wrapper, estensione GNOME, configurazione, cache e COPR `dlk/autokey` sono stati eliminati. Il backup completo e' in `/home/daniele/backups/autokey/582941-20260714T222324+0200`.
- Espanso 2.3.0 e' installato dal pacchetto Terra `espanso-wayland`, con servizio utente `espanso.service` abilitato e attivo. La configurazione e' in `~/.config/espanso`, usa backend Clipboard, layout `it` e contiene 3 match migrati da AutoKey. Gli script/macro AutoKey e le frasi senza abbreviazione non sono stati migrati perche' fuori dallo scope di espansione testo.
