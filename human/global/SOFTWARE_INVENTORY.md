...
- Fedora Diagnostics 1.1.0 e' installato in `/usr/local/lib/fedora-diagnostics` con CLI `/usr/local/bin/fedora-diagnostics`. La modalita' `--fan-analysis` esporta storico CPU/GPU/ventole/potenza/processi; il collector textfile ogni 30 secondi non apre porte, limita le top list a dieci nomi eseguibile e non legge command line o argomenti.
...
- Il 2026-07-14 AutoKey ha trattenuto un device evdev scomparso e ripetuto ENODEV senza backoff, consumando un core e alimentando le ventole. Un riavvio controllato ha ricostruito i device e ripristinato circa 0,2% CPU; il difetto resta mitigato, non corretto upstream (attivita' `614283`).
