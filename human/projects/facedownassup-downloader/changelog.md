...
- Prompt `#826194`: added global `porndownloader` script and symlink in `~/.local/bin`; default quality is 720p with Firefox cookies from `/home/daniele/.config/mozilla/firefox/50b1zmic.default-release`, output to `~/Videos/porndownloader`, state/history in `~/.local/state/porndownloader`, intelligent filename fallback, SQLite history, source-URL dedupe, `--formats`, `--dry-run`, `--list`, `--output-dir`, and `--keep-partials`. Real `--formats` test succeeded; real 720p download passed fragment `72` but failed at fragment `175` with `404`, leaving no final file/history row.
