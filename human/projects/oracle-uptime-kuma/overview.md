...
Dal 2026-06-15 l'interfaccia amministrativa e' di nuovo raggiungibile da browser su:

```text
http://150.230.148.128:3001/dashboard
```

Kuma resta legato localmente a `127.0.0.1:3002`; Nginx espone `150.230.148.128:3001` come reverse proxy completo per dashboard, Socket.IO e push esistenti `/api/push/...`.
...
- proxy pubblico: Nginx su `:3001`, dashboard completa e `/api/push/`.
...
- Il login/admin pubblico su `http://150.230.148.128:3001` e' HTTP semplice: usare con attenzione fino a migrazione HTTPS/restrizione accessi.
